...
import { X, Save, Building, User, Check, PlusCircle, MinusCircle } from 'lucide-react';

const SalesPointForm = ({ salesPoint, allUsers, onSave, onClose }) => {
  const [selectedUserIds, setSelectedUserIds] = useState(salesPoint.associatedUsers.map(u => u.id));

  useEffect(() => {
    setSelectedUserIds(salesPoint.associatedUsers.map(u => u.id));
  }, [salesPoint]);
  ...
  const handleSubmit = (e) => {
    ...
    const updatedSalesPoint = {
      ...salesPoint,
      associatedUsers: allUsers.filter(user => selectedUserIds.includes(user.id))
    };
    onSave(updatedSalesPoint);
  };
      ...
      >
        ...
        <div className="flex items-center justify-between p-2 border-b border-gray-200 mb-6">
          <h2 className="text-2xl font-bold text-gray-900">
            Asociar Usuarios a {salesPoint.name}
          </h2>
          <motion.button
            onClick={onClose}
